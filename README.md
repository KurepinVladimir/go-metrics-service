...
Increment 24 — Асимметричное шифрование (краткое описание)
В проект добавлена поддержка асимметричного шифрования трафика между агентом и сервером с использованием RSA.

Добавлены флаги и переменные окружения -crypto-key / CRYPTO_KEY
— агент принимает публичный ключ, сервер — приватный.

Агент перед отправкой:
-формирует JSON,
-считает HMAC (если включён),
-сжимает данные gzip,
-шифрует тело RSA-OAEP публичным ключом.

На сервере реализован middleware DecryptRSA, который:
-расшифровывает тело запроса приватным ключом,
-передаёт результат дальше в цепочку обработки.

Для JSON-эндпоинтов добавлена цепочка middleware:
RSA-decrypt → gzip → ValidateHashSHA256

Старые текстовые эндпоинты не изменены.
Добавлен юнит-тест rsa_decrypt_test.go, покрывающий сценарии:
-без приватного ключа (pass-through),
-с приватным ключом (корректная расшифровка).

Итог: Обмен данными между агентом и сервером стал защищён RSA-шифрованием, при этом сохранилась совместимость с gzip и HMAC.
